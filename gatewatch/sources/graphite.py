...
MERGE_PROBABILITY = {
    'format': 'json',
    'from': '-10seconds',
    'target': 'alias(secondYAxis(lineWidth(asPercent(transformNull(multiplySeries(transformNull(movingAverage(divideSeries(stats.zuul.pipeline.gate.job.gate-tempest-dsvm-neutron.SUCCESS,sum(stats.zuul.pipeline.gate.job.gate-tempest-dsvm-neutron.{SUCCESS,FAILURE})),%2721week%27),1),transformNull(movingAverage(divideSeries(stats.zuul.pipeline.gate.job.gate-tempest-dsvm-full.SUCCESS,sum(stats.zuul.pipeline.gate.job.gate-tempest-dsvm-full.{SUCCESS,FAILURE})),%2721week%27),1),transformNull(movingAverage(divideSeries(stats.zuul.pipeline.gate.job.gate-grenade-dsvm.SUCCESS,sum(stats.zuul.pipeline.gate.job.gate-grenade-dsvm.{SUCCESS,FAILURE})),%2721week%27),1),transformNull(movingAverage(divideSeries(stats.zuul.pipeline.gate.job.gate-tempest-dsvm-neutron-large-ops.SUCCESS,sum(stats.zuul.pipeline.gate.job.gate-tempest-dsvm-neutron-large-ops.{SUCCESS,FAILURE})),%2721week%27),1),transformNull(movingAverage(divideSeries(stats.zuul.pipeline.gate.job.gate-tempest-dsvm-large-ops.SUCCESS,sum(stats.zuul.pipeline.gate.job.gate-tempest-dsvm-large-ops.{SUCCESS,FAILURE})),%2721week%27),1),transformNull(movingAverage(divideSeries(stats.zuul.pipeline.gate.job.gate-tempest-dsvm-postgres-full.SUCCESS,sum(stats.zuul.pipeline.gate.job.gate-tempest-dsvm-postgres-full.{SUCCESS,FAILURE})),%2721week%27),1))),1),2)),%27Patch%20Pass%20Chance%20Moving%20Average%27)'}
